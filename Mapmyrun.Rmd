...
unrounded_all_coordinated <- fread("C:/Users/Jesse/Programming/R/Map My Run Redone/Data/all_coordinates_not_rounded_or_unfiltered 1.csv")

count_all_unrounded_coordinates <- unrounded_all_coordinated %>%
  group_by(V1, V2) %>%
  summarise(n=n())
...
colnames(count_all_unrounded_coordinates) <- c("lon", "lat", "n")
...
count_all_unrounded_coordinates %>%
  arrange(desc(n)) %>%
  head(50)
#top is only 22, no bueno. But I am only doing this in order to map it and see what it looks like. 

#if i don't round, the txt file has like 10 decimals, which is too much to handle. I think the gps files don't go past 5 decimal points anyway. 

count_all_unrounded_coordinates_lat_lon <- count_all_unrounded_coordinates_lat_lon %>%
  mutate_at(c("lat", "lon"), funs(round(., 5)))

#Save!
write_csv(
  count_all_unrounded_coordinates, 
  "count_all_unrounded_coordinates_not_including_non_Baltimore_address.csv")

write.table(count_all_unrounded_coordinates_lat_lon,
            "unrounded_coords.txt",row.names = FALSE, col.names= FALSE, dec=".",sep=";",eol="\\n")
```
